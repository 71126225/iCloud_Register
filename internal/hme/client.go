...
type Alias struct {
	...
	ForwardTo   string `json:"forwardTo,omitempty"` // 转发目标邮箱 (账号级设置)
}
...
type Client struct {
	...
	pendingAuth *authState // BeginLogin 触发 2FA 后保存的中间状态
}
...
//
// 对照 icloud-hide-my-email-browser-extension 的 updateForwardToHme:
// POST {premiummailsettings}/v1/hme/updateForwardTo {"forwardToEmail": "..."}
// 目标地址必须是账号已验证的地址 (如账号的 @icloud.com 邮箱)。
func (c *Client) UpdateForwardTo(forwardToEmail string) error {
	if err := c.resolveService(); err != nil {
		return err
	}
	c.log("修改转发地址 → %s ...", forwardToEmail)
	payload := map[string]string{"forwardToEmail": forwardToEmail}
	body, err := c.request("POST", c.serviceURL+"/v1/hme/updateForwardTo", payload, 0, 2)
	if err != nil {
		return err
	}
	if !gjson.Get(body, "success").Bool() {
		return fmt.Errorf("修改转发地址失败: %s", gjson.Get(body, "error.errorMessage").String())
	}
	return nil
}
